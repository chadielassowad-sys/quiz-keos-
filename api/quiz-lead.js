 ...
 *
 * Creates/updates a HubSpot contact and (optionally) stores top quiz errors as a note.
 */
...
function sendJson(res, statusCode, payload) {
  ...
  // Helps if you ever call this endpoint cross-origin.
...
async function upsertContact(payload) {
  ...
  if (contactId && errors.length > 0) {
    const lines = errors.slice(0, 10).map((e, i) => {
      const section = e.section || "";
      const qText = e.question || "";
      const selected = e.selected || "";
      const correct = e.correct || "";
      return `${i + 1}) [${section}] ${qText}\n- Reponse candidat: ${selected}\n- Bonne reponse: ${correct}`;
    });
...
module.exports = async (req, res) => {
  try {
    ...
    if (!HUBSPOT_TOKEN) {
      return sendJson(res, 500, { ok: false, error: "Missing HUBSPOT_PRIVATE_APP_TOKEN" });
    }
